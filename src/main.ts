...
const COMMANDS = ["help", "about", "projects", "whoami", "repo", "resume","banner", "clear","skills"];
...
function commandHandler(input : string) {
  ...
  switch(input) {
    ...
    case 'banner':
      if(bareMode) {
        writeLines(["DeepShell v1.0.0", "<br>"])
        break;
      }
      writeLines(BANNER);
      break;
    ...
    case 'skills':
      if(bareMode) {
        writeLines(["I don't want you to break the other SKILLS.", "<br>"])
        break;
      }
      writeLines(SKILLS);
      break
